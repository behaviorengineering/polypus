// Package chatsmoke runs L1 transport probes against a Polypus gateway.
package chatsmoke
...
// Run parses flags from args (typically os.Args[1:]) and executes chat smoke probes.
func Run(args []string) int {
	fs := flag.NewFlagSet("polypus-chat-smoke", flag.ContinueOnError)
	fs.SetOutput(os.Stderr)
	baseURL := fs.String("base-url", envOr("POLYPUS_BASE_URL", "http://127.0.0.1:1320"), "Polypus gateway base URL")
	model := fs.String("model", "", "model id (backend/downstream); required unless -list")
	list := fs.Bool("list", false, "print default chat smoke models and exit")
